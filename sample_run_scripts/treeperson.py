...
if __name__== "__main__":
  ...
  ### Dataset info
  dataset_name = "treeperson"
  dataset_path = "/enter/path/here/datasets/gqa/images"
  target_attr = "has_person"
  group_labels = ["NoPerson/Trees", "NoPerson/Building", "Person/Trees", "Person/Building"]
  ...
  acquisition_functions = ["uncertainty", "random"]
  seed_set_size = 40
  seed_set_is_balanced=True
  acq_num_query_total = 40*8
  acq_num_query_initial = 5
  ...
  for vs in valid_splits:    
    for i1, acq_f in enumerate(acquisition_functions):
      for i2, sft in enumerate(stop_from_train):
        s = "stop_from_train" if sft is not None else "stop_from_val"
        overall_mean = {
          int(key): overall_accuracies_over_time[vs][key][:,i1,i2].mean(axis=0) for key in overall_accuracies_over_time[vs]
        }
        group_mean = {
          int(key): group_accuracies_over_time[vs][key][:,i1,i2,...].mean(axis=0) for key in group_accuracies_over_time[vs]
        }
        plot_acc_over_time(overall_mean, [f"{vs}_overall accuracy"], logdir_base, f"{vs}_overall_accuracies_over_time_{acq_f}_{s}", f"{vs} accuracies over training")
        plot_acc_over_time(group_mean, group_labels, logdir_base, f"{vs}_group_accuracies_over_time_{acq_f}_{s}", f"{vs} accuracies over training")
